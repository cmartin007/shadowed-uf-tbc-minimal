...
- **Party + target out-of-range fade:** `modules/range.lua` fades frames to `oorAlpha` (0.4) when OOR; restores to `inAlpha` (1.0) when in range. Enabled for party and target in `defaultlayout.lua`. Two strategies: friendly units use `UnitInRange` (~40 yd); hostile units use `IsSpellInRange` with `CLASS_RANGE_SPELL[playerClass]` (Fireball/Wrath/Shadow Bolt etc.). `nil` from either API must be treated as in-range — never fade on unlearned spells or uncheckable units. Warriors/rogues have no `CLASS_RANGE_SPELL` entry and skip hostile fading. Config: `range = {enabled, oorAlpha, inAlpha}` in AceDB defaults (all non-player units). Uses `SetRangeAlpha` / `DisableRangeAlpha` on frames (`modules/units.lua`).
